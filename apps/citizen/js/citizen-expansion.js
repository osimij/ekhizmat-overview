...
export function initCitizenExpansion(ctx){
  ...
  function renderApplications(){
    ...
    if(state.appView==="detail"){
      ...
      root.innerHTML=`<button class="back" data-app-action="list">${icon("i-chev-l")}<span>${c.back}</span></button><article class="application-detail"><div class="application-detail__head"><div><span class="eyebrow">${localizedCategory(a.category)}</span><h2>${a.name}</h2><p class="muted">${c.ref} №${a.id} · ${a.date}</p></div><span class="status-pill status-pill--${statusClass(a.status)}">${c[a.status]}</span></div><div class="application-path"><span>${icon('i-building')}</span><div><small>${c.currentAgency}</small><strong>${a.currentAgency}</strong><small>${c.expected} ${a.eta}</small></div></div>${a.reason?`<div class="note-warn"><strong>${c.reason}</strong><p>${a.reason}</p></div>`:""}<h3>${c.history}</h3><ol class="application-history">${stages.map((label,index)=>`<li class="${index<a.stage?'done':index===a.stage?'current':''}"><span>${icon(index<a.stage?'i-check':index===a.stage?'i-clock':'i-dots')}</span><div><b>${label}</b><small>${index<=a.stage?`${a.date}, ${String(9+index).padStart(2,'0')}:1${index}`:'—'}</small></div></li>`).join('')}</ol></article>`;
      return;
    }
  ...
  function renderTracking(){
    ...
    root.innerHTML=`<section class="under-review" aria-labelledby="underReviewH"><div class="pane-heading"><div><h2 id="underReviewH">${c.trackTitle}</h2><p class="lead-s">${c.trackLead}</p></div><span class="status-pill status-pill--info">${active.length}</span></div><div class="tracking-cards">${active.map(a=>`<article class="tracking-card ${a.status==='action'?'tracking-card--warning':''}"><button type="button" data-track-id="${a.id}" aria-label="${a.name}"><span class="tracking-card__head"><span><strong>${a.name}</strong><small>№${a.id} · ${a.currentAgency}</small></span><span class="status-pill status-pill--${statusClass(a.status)}">${c[a.status]}</span></span><span class="mini-stepper" aria-label="${c.history}">${labels.map((label,index)=>`<span class="mini-stepper__step ${index<a.stage?'is-done':''}" ${index===a.stage?'aria-current="step"':''}><i>${index<a.stage?'✓':index+1}</i><em>${label}</em></span>`).join('')}</span><span class="tracking-card__eta">${c.expected} ${a.eta}</span></button>${a.status==='action'?`<button class="btn btn-sec btn-sm" data-track-id="${a.id}">${c.action}</button>`:''}</article>`).join('')}</div></section>`;
  }
