...
const DEFAULT_STATE = {
  ...
  formFields:[
    ...
    {id:'address',label:{ru:'Юридический адрес',tg:'Суроғаи ҳуқуқӣ'},type:'text',required:false},
  ],
  review:null,
  publishedAt:null,
  audit:MAIN_AUDIT,
...
const COPY = {
  ...
  tg:{
    ...
    queueTitle:'Хизматҳо дар кор',queueHint:'Барои дидани намуди пурра ва амалҳои дастрас сатрро кушоед.',search:'Ҷустуҷӯ аз рӯи хизмат, муаллиф ё рамз…',filterAgency:'Идора',any:'Ҳамаи идораҳо',serviceColumn:'Хизмат',createdBy:'Хизматро сохт',authorColumn:'Муаллиф',submittedAt:'Фиристода шуд',status:'Ҳолат',openService:'Кушодани хизмат',found:n=>`${n} хизмат`,
...
const SERVICE_RECORDS = [
  ...
  },
  {
    ...
    fields:[{label:{ru:'Кадастровый номер',tg:'Рақами кадастрӣ'},type:'text',required:true},{label:{ru:'Цель получения выписки',tg:'Мақсади гирифтани иқтибос'},type:'select',required:true},{label:{ru:'Документ заявителя',tg:'Ҳуҷҷати дархосткунанда'},type:'file',required:false}],
  },
  {
    ...
    fields:[{label:{ru:'Адрес объекта',tg:'Суроғаи иншоот'},type:'text',required:true},{label:{ru:'Кадастровый номер',tg:'Рақами кадастрӣ'},type:'text',required:true},{label:{ru:'Проектная документация',tg:'Ҳуҷҷатҳои лоиҳавӣ'},type:'file',required:true},{label:{ru:'Правоустанавливающий документ',tg:'Ҳуҷҷати тасдиқкунандаи ҳуқуқ'},type:'file',required:false}],
  },
  {
    ...
    fields:[{label:{ru:'ПИН заявителя',tg:'РМА-и дархосткунанда'},type:'text',required:true},{label:{ru:'Адрес регистрации',tg:'Суроғаи бақайдгирӣ'},type:'text',required:false}],
  },
];
...
export function dispatchLowCode(event,payload={}){
  /* Ведомство создаёт форму с нуля: общий процесс должен встать в черновик, а
     не в демо-состояние «на рассмотрении», иначе только что созданная форма
     открывается уже запертой для правки. */
  if(event==='NEW_DRAFT'){
    state={...DEFAULT_STATE,status:'draft',serviceVersion:'1.0',serviceName:{ru:'',tg:''},formFields:[],audience:['person'],comments:[],review:null,publishedAt:null,audit:[],queueStates:clone(INITIAL_QUEUE_STATES)};
...
function queueMarkup(){
  ...
  /* §6 empty state: the role hint appears when there is nothing else to look at */
  const empty=`<div class="reg-empty lc-queue-empty"><svg><use href="/design-system/assets/icons.svg#i-inbox"/></svg><b>${x.emptyTitle}</b><p>${x.emptyBody}</p><p class="lc-queue-empty__hint">${roleHint()}</p></div>`;
  return `<section class="panel lc-queue-panel"><div class="lc-queue-toolbar"><h2>${x.queueTitle}</h2><span class="lc-result-count">${x.found(records.length)}</span><label class="ekh-filter" for="lcAgencyFilter"><span class="ekh-filter__label"><svg aria-hidden="true"><use href="/design-system/assets/icons.svg#i-filter"/></svg>${x.filterAgency}</span><span class="ekh-filter__field"><select id="lcAgencyFilter" data-lc-filter="agency" aria-label="${x.filterAgency}"><option value="all">${x.any}</option>${agencies}</select><svg aria-hidden="true"><use href="/design-system/assets/icons.svg#i-chev-d"/></svg></span></label></div>${records.length?`<div class="lc-queue-table" role="table" aria-label="${x.queueTitle}"><div class="lc-queue-table__head" role="row"><span role="columnheader">${x.serviceColumn}</span><span role="columnheader">${x.authorColumn}</span><span role="columnheader">${x.submittedAt}</span><span role="columnheader">${x.version}</span><span role="columnheader">${x.status}</span><span aria-hidden="true"></span></div><div class="lc-queue-table__body">${records.map(queueRowMarkup).join('')}</div></div>`:empty}</section>`;
}
function queueRowMarkup(record){
  const x=c(),workflow=workflowFor(record.id);
  return `<button class="lc-queue-row" type="button" data-lc-service="${record.id}" role="row" aria-label="${x.openService}: ${escapeHtml(localized(record.name))}"><span class="lc-queue-service" role="cell"><span class="lc-queue-icon"><svg><use href="/design-system/assets/icons.svg#i-cat-cert"/></svg></span><span><b>${escapeHtml(localized(record.name))}</b><small>${escapeHtml(localized(record.agency))} · ${escapeHtml(record.code)}</small></span></span><span class="lc-queue-person" role="cell"><b>${escapeHtml(record.creator)}</b></span><time role="cell">${escapeHtml(record.submittedAt)}</time><span class="lc-version" role="cell">v${escapeHtml(workflow.version)}</span><span role="cell">${statusBadge(workflow.status)}</span><svg class="lc-row-arrow" aria-hidden="true"><use href="/design-system/assets/icons.svg#i-chev-r"/></svg></button>`;
}

function serviceOverviewMarkup(record){
  const x=c(),fields=recordFields(record);
  /* the detail head already carries the name and the description — the panel
     opens at the facts (design-guide rules 4 and 6) */
  return `<section class="panel lc-service-overview"><div class="lc-facts"><div><span>${x.agency}</span><b>${escapeHtml(localized(record.agency))}</b></div><div><span>${x.serviceCode}</span><b>${escapeHtml(record.code)}</b></div><div><span>${x.delivery}</span><b>${escapeHtml(x[record.delivery])}</b></div><div><span>${x.price}</span><b>${escapeHtml(record.price==='free'?x.free:localized(record.price))}</b></div></div><div class="lc-overview-section"><div class="lc-section-heading"><h3>${x.applicationForm}</h3><span class="metachip">${fields.length}</span></div><div class="lc-form-preview">${fields.map((field,index)=>`<div class="lc-preview-field"><span class="lc-preview-field__number">${String(index+1).padStart(2,'0')}</span><span class="lc-preview-field__icon"><svg><use href="/design-system/assets/icons.svg#${fieldTypeIcon(field.type)}"/></svg></span><span><b>${escapeHtml(localized(field.label))}</b><small>${field.required?x.required:x.optional}</small></span></div>`).join('')}</div></div><div class="lc-overview-section"><div class="lc-section-heading"><h3>${x.route}</h3></div><ol class="lc-route"><li><span>1</span><p>${x.route1}</p></li><li><span>2</span><p>${x.route2}</p></li><li><span>3</span><p>${x.route3}</p></li></ol></div><div class="lc-overview-section lc-notification"><span class="lc-notification__icon"><svg><use href="/design-system/assets/icons.svg#i-bell"/></svg></span><div><h3>${x.notifications}</h3><p>${x.notificationText}</p></div></div></section>`;
}
...
function renderReview(){
  ...
  /* one h1 per view: the list is titled by the page header, the detail by the
     service it opened (design-guide rules 4 and 6) */
  root.innerHTML=reviewView==='detail'&&selected
    ? detailMarkup(selected)
    : `<header class="lc-review-head"><h1>${x.reviewQueue}</h1><p>${x.reviewLead}</p></header><div class="lc-review-tabs" role="tablist">${tabs.map(tab=>`<button class="tab" type="button" role="tab" data-lc-tab="${tab.id}" aria-selected="${reviewTab===tab.id}" tabindex="${reviewTab===tab.id?'0':'-1'}">${tab.label}</button>`).join('')}</div>${queueMarkup()}`;
}
...
function bind(){
  ...
  /* the search input lives in the stable top bar, so it survives every re-render */
  document.querySelector('#reviewSearch')?.addEventListener('input',event=>{reviewQuery=event.target.value;renderReview();});
