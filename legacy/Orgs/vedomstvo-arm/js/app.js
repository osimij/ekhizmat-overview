...
(function () {
  ...
  var S = {
    authed: false,
    loginStep: 1,
    lang: localStorage.getItem('varm-lang') || 'ru',
    theme: localStorage.getItem('varm-theme') || 'light',
    view: 'queue',
    cardId: null,
    cardTab: 'overview',
    apps: [],
  ...
  function ic(name, cls) { return '<svg class="icon ' + (cls || '') + '" aria-hidden="true"><use href="#' + name + '"/></svg>'; }
  ...
  function fmtDate(ts) {
    var dt = new Date(ts);
    var dd = String(dt.getDate()).padStart(2, '0'), mm = String(dt.getMonth() + 1).padStart(2, '0');
    return dd + '.' + mm + '.' + dt.getFullYear();
  }
  function fmtDateTime(ts) {
    var dt = new Date(ts);
    return fmtDate(ts) + ' ' + String(dt.getHours()).padStart(2, '0') + ':' + String(dt.getMinutes()).padStart(2, '0');
  }
  function fmtAgo(ts) {
    var diff = now() - ts;
  ...
  function persist() {
    try { localStorage.setItem(LS_KEY, JSON.stringify({ apps: S.apps, notifs: S.notifs })); } catch (e) {}
  }
  function loadData() {
    try {
      var raw = localStorage.getItem(LS_KEY);
      if (raw) { var o = JSON.parse(raw); if (o && o.apps && o.apps.length) { S.apps = o.apps; S.notifs = o.notifs || []; return; } }
    } catch (e) {}
    var m = materialize(); S.apps = m.apps; S.notifs = m.notifs; persist();
  }
  function resetData() { localStorage.removeItem(LS_KEY); var m = materialize(); S.apps = m.apps; S.notifs = m.notifs; persist(); }
  ...
  function rowQueue(a) {
    ...
    return '<div class="q-row ' + s.hue + (sel ? ' is-selected' : '') + '" data-act="open-card" data-id="' + a.id + '" tabindex="0" role="button" aria-label="' + esc(aria) + '">' +
      ...
      '<span class="q-sla"><span class="sla sla--' + st + '" data-sla data-due="' + a.dueAt + '" title="' + esc(slaWord(st)) + '" aria-label="' + esc(slaWord(st)) + '"><span class="dot"></span>' +
        '<svg class="icon icon--16 sla__ico" aria-hidden="true"><use href="#i-clock"/></svg>' +
  ...
  function openModal(html, cls) {
    ...
    var focusable = mod.querySelector('input:not([type=hidden]),select,textarea,button');
    (focusable || mod).focus();
  }
  function closeModal() {
    S.modal = null; overlayEl().innerHTML = '';
  ...
  function closeLayers(all) { closePop(); if (all) { S.modal = null; if (overlayEl()) overlayEl().innerHTML = ''; } }
  ...
  document.addEventListener('click', function (e) {
    ...
    switch (act) {
      ...
      case 'lang': S.lang = S.lang === 'ru' ? 'tg' : 'ru'; localStorage.setItem('varm-lang', S.lang); renderApp(); return;
  ...
  function toggleTheme() {
    ...
    localStorage.setItem('varm-theme', S.theme);
