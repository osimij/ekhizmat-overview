...
| Platform launcher | Full-viewport `--bg` canvas. The decision is a **centered intrinsic cluster**, not a stretched `--w-launcher` / `--maxw` column: logo (mark only, above the title) + display `h1` (`--fs-44`) + one orientation line on the left; destinations on the right as a **single stack of four compact `--panel` rows** (category hue tile + name + audience, `--line-in` hairline). Language and theme sit as quiet **bottom-left chrome** (globe + current language, slash, theme icon) because this page has no profile — they follow the content in the DOM so visual order matches reading order. Chrome inset is `clamp(--s-4, 2vw, --s-8)`, same as login. Below ~960px the title stacks above the list and the cluster width matches the destination column (`26rem`); the four rows stay a single column at every width. No top header or wordmark, no 2×2, no bordered description cards, no copy that restates the destination, no decorative arrows. *Why: the job is to pick a workplace. A 2×2 fights the one-decision reading order; stretching the cluster to 1440 opened a dead gutter between title and list; a top header duplicated the logo that already sits with the title. An intrinsic 24rem / 26rem pair stays together from 961px to ultrawide; stacking below 960px keeps the same rows tappable on the phone.* | 2×2 destination grid; bordered description cards with corner arrows; `--maxw`-capped one-column stack; uncapped edge-pinned split; top header with wordmark + preferences |
...
46. A platform chooser built as a 2×2 of cards, a stretched `--w-launcher` split, a top header with wordmark + preferences, or an uncapped edge-pinned layout → centered intrinsic cluster: logo + title left, four compact `--panel` rows stacked right; language/theme as slash-separated bottom-left chrome. Descriptions, arrows, and a second logo in a header go.
