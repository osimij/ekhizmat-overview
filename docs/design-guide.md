...
| Workstation login | Shared `.login` gate (`design-system/css/patterns.css`): logo + product name, one 420px card, floating labels, pill fields at `--h-btn-l`, credentials then MFA. Step 1 primary is Continue; step 2 is Enter with a Back control. No "step N of M" chrome — the control change is the progress. Canvas is `--login-bg`: light `--hero` wash, dark `--pure-black` — not `--hero-dark`, which is the navy citizen-artwork sky. *Why: specialist consoles share one morning ritual; a second login that looks like a form wizard teaches operators that the products are different. A navy wash behind a dark panel reads as an unfinished canvas; black lets the card be the only surface.* | Label-above fields; "step 1 of 2" inside the card; two screens both titled Enter |
...
| Workstation live-scan | A quiet `--field` rounded square with the face icon centered. While scanning, one `--blue` dotted SVG stroke marches via `stroke-dashoffset` (~4s linear). The caption is the instruction. Reduced motion: the same dotted stroke, static. *Why: Face ID is a tens-of-times-a-day operator action — a traveling bar, pulsing corner brackets, and a glow are three decorations of “live”. One slow dashed stroke is the live mark; anything else is chrome the operator will hate by noon.* | Corner-bracket pulse + traveling scanline + `box-shadow` glow |
...
30. A live Face ID / biometric scan with a traveling bar, pulsing brackets, or glow → one `--blue` marching dotted stroke on the frame; the caption carries the instruction. Reduced motion keeps the dotted stroke static. Do not also pulse the icon.
