...
2. **Row hover geometry: records bleed, options float.** Data rows in tables and registries (`.svc-row`, `.tbl`, `.data-table`, queue rows) carry the panel's inline padding themselves and hover **edge to edge with square corners** — the highlight touching the edges says "this whole record". Menu items, sidebar items, and option tiles sit *inside* their container's padding and hover as **rounded islands** (`--r-s`/8px). *Why: a square bleed inside a rounded popover collides with its corners; a floating island inside a wide table makes the row look narrower than its data.*
...
34. Hover highlight in the wrong geometry → records bleed square edge-to-edge; menu/nav/option items hover as rounded inset islands (§5 casebook 2). Never mix the two in one context.
