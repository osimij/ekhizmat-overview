...
| Portal sign-in | The lock overlay language on a dismissible dialog: `.overlay.s-locked` scrim, brand top-left, `--panel` card at `--r-lock`, S0 floating-label fields at `--login-scale`. Phone, then six-cell MFA (any 6 digits in demo). Close is a ghost action on the phone step; Back returns from MFA. No workstation session legend. The portal behind blurs (`.is-blurred` on `header`/`main`). *Why: the citizen is already on a live page, so lock composition is the interrupt and login fields are the credential language — a generic label-above `.modal` was a third dialect.* | Generic `.modal` with a label-above phone field; signing in without MFA |
...
51. A citizen/portal sign-in that still uses a generic label-above `.modal` → the lock overlay plus S0 login fields: `.overlay.s-locked`, brand on the scrim, floating phone, then six-cell MFA, ghost Close/Back. Do not copy the workstation session legend. The portal behind uses `.is-blurred`.
52. A login MFA error that paints on arrival (or shakes before the operator has submitted a code) → one form per step; keep the error `hidden` until that step’s own submit fails. Do not handle Enter on credentials and MFA in the same keydown — native form submit is the ЦОН contract.
53. A builder preview of document issuance that reuses a delivery-option row (`.opt`) or a 36px QR → the issued certificate as the citizen presents it from the wallet: `--doc-paper` surface, holder and facts, electronic seal, and a scannable QR. `.opt` is for choosing a delivery method; the issue step’s job is the document itself. *Why: an option tile with a tiny QR reads as an unfinished list, not as the result the pipeline just produced — and it cannot show the seal, the household, or that the original is digital.*
