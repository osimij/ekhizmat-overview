/* ===================== OPERATOR — interactions =====================
   Extracted from the shared blueprint.js for the standalone OPERATOR bundle.
   Declarative, data-attribute driven so each page wires behaviour in markup:
   theme toggle, page-nav, language switcher + engine (tg↔ru), toast, modals
   (focus-trap), pressed groups, tabs, the multi-step wizard, faux-QR, and the
   operator rail builder. No admin/citizen code travels with this file.
   ================================================================== */
(function(){
"use strict";
...
var PAGES=[
  ...
  ["index.html",        "◆ Ҳама экранҳои оператор",        "◆ Все экраны оператора"],
  ["op-login.html",     "Оператор · оғози баст",           "Оператор · начало смены"],
  ["op-queue.html",     "Оператор · навбат",               "Оператор · очередь"],
  ["op-verify.html",    "Оператор · сессияи хизматрасонӣ", "Оператор · сессия обслуживания"]
];
...
/* ---------- operator rail (in-person service-window console) ----------
   Its own nav + a window-aware foot with sign-out. */
var OP_NAV = [
  ["__label","Кор"],
  ["queue","Навбат","i-inbox","12"],
  ["verify","Сессияи ҷорӣ","i-check"]
];
$$("[data-op-rail]").forEach(function(rail){
  var active = rail.getAttribute("data-active");
  var h = '<a class="adm-brand" href="op-queue.html"><svg class="mark" aria-hidden="true"><use href="#i-logo"/></svg><b>eKhizmat</b><span class="tag">Оператор</span></a>';
  OP_NAV.forEach(function(it){
    if (it[0]==="__label"){ h += '<div class="adm-nav-label">'+it[1]+'</div>'; return; }
    var cur = it[0]===active;
    var href = it[0]==="queue" ? "op-queue.html" : "op-verify.html";
    h += '<a class="an" href="'+href+'"'+(cur?' aria-current="true"':'')+'>'+
         '<svg><use href="#'+it[2]+'"/></svg><span>'+it[1]+'</span>'+
         (it[3]?'<span class="cnt hot">'+it[3]+'</span>':'')+'</a>';
  });
  h += '<div class="spacer"></div>';
  h += '<a class="adm-me" href="#" onclick="return false"><span class="av">АК</span><span class="nm"><b>Аброр Каримов</b><span>Оператор · Тиреза 4</span></span></a>';
  h += '<a class="an" href="op-login.html"><svg><use href="#i-out"/></svg><span>Пӯшидани баст</span></a>';
  rail.innerHTML = h;
});
