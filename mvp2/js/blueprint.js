/* ===================== BLUEPRINT — shared interactions =====================
   Declarative, data-attribute driven so each page wires behaviour in markup.
   Mirrors the patterns in app.js (modal focus-trap, stepper, toast, seeded QR).
   ========================================================================== */
(function(){
"use strict";
...
var THEME="ekh-theme", mq=matchMedia("(prefers-color-scheme: dark)");
function setTheme(){
...
/* ---------- blueprint page nav (one shared list, injected into every toolbar) ----------
...
var PAGES=[
  ...
  ["index.html",        "◆ Ҳама экранҳо (gallery)",        "◆ Все экраны (gallery)"],
  ["components.html",    "◆ Системаи компонентҳо (UI kit)", "◆ Система компонентов (UI kit)"],
  ["map-citizen.html",  "Харита · сафари корбар",          "Карта · путь пользователя"],
  ["map-admin.html",    "Харита · админ",                  "Карта · админ"],
  ["service.html",      "Корбар · сафҳаи хизмат",          "Пользователь · страница услуги"],
  ["apply.html",        "Корбар · шакли ариза (шаблон)",   "Пользователь · форма заявления (шаблон)"],
  ["track.html",        "Корбар · пайгирии ариза",         "Пользователь · отслеживание заявления"],
  ["document.html",     "Корбар · ҳуҷҷати тайёр",          "Пользователь · готовый документ"],
  ["admin-login.html",  "Админ · вуруд",                   "Админ · вход"],
  ["admin-dashboard.html","Админ · лавҳаи идора",          "Админ · панель управления"],
  ["admin-inbox.html",  "Админ · аризаҳо",                 "Админ · заявления"],
  ["admin-review.html", "Админ · баррасии ариза",          "Админ · рассмотрение заявления"],
  ["admin-users.html",  "Админ · корбарон",                "Админ · пользователи"],
  ["admin-user-new.html","Админ · илова кардани корбар",   "Админ · добавить пользователя"],
  ["admin-services.html","Админ · хизматрасониҳо",         "Админ · услуги"],
  ["admin-roles.html",  "Админ · нақшҳо ва ҳуқуқ",         "Админ · роли и права"],
  ["admin-settings.html","Админ · танзимот",               "Админ · настройки"]
];
function navLang(){ var l="tg"; try{ l=localStorage.getItem("ekh-lang")||"tg"; }catch(e){} return l; }
...
var LANG_SWITCH='<div class="dd lang">'+
  '<button class="dd-btn" type="button" aria-haspopup="listbox" aria-expanded="false" aria-label="Забон / Language">'+
    '<svg class="gl"><use href="#i-globe"/></svg><span id="langCur" data-no-i18n>ТҶ</span><svg class="caret"><use href="#i-chev-d"/></svg></button>'+
  '<div class="dd-menu" role="listbox" aria-label="Забон / Language">'+
    '<div class="dd-label" aria-hidden="true">Забон</div>'+
    '<button role="option" data-lang="tg" aria-selected="true"><span data-no-i18n>Тоҷикӣ</span><svg><use href="#i-check"/></svg></button>'+
    '<button role="option" data-lang="ru" aria-selected="false"><span data-no-i18n>Русский</span><svg><use href="#i-check"/></svg></button>'+
...
$$(".hdr .hdr-in").forEach(function(hin){
  ...
  acts.innerHTML='<button class="icon-btn" id="themeBtn2" data-theme-toggle aria-label="Тема">'+
    '<svg class="th-moon"><use href="#i-moon"/></svg><svg class="th-sun"><use href="#i-sun"/></svg></button>';
...
/* ---------- multi-step flows (apply stepper / add-user wizard) ---------- */
...
function addFile(dz){
  ...
  row.innerHTML='<span class="fi"><svg><use href="#i-paperclip"/></svg></span>'+
    '<div class="fn"><b>'+nm+'</b><span>бор мешавад…</span><div class="prog"><i style="width:8%"></i></div></div>'+
    '<button class="fx" data-file-remove aria-label="Бекор кардан"><svg><use href="#i-x"/></svg></button>';
...
/* ---------- admin rail (one source of truth for all admin pages) ---------- */
var ADM_NAV = [
  ["__label","Кор"],
  ["dashboard","Лавҳаи идора","i-dash"],
  ["inbox","Аризаҳо","i-inbox","12"],
  ["users","Корбарон","i-users"],
  ["__label","Танзим"],
  ["services","Хизматрасониҳо","i-cat-cert"],
  ["roles","Нақшҳо ва ҳуқуқ","i-role"],
  ["settings","Танзимот","i-gear"]
];
$$("[data-adm-rail]").forEach(function(rail){
  var active = rail.getAttribute("data-active");
  var h = '<a class="adm-brand" href="admin-dashboard.html"><svg class="mark" aria-hidden="true"><use href="#i-logo"/></svg><b>eKhizmat</b><span class="tag">Админ</span></a>';
  ADM_NAV.forEach(function(it){
    if (it[0]==="__label"){ h += '<div class="adm-nav-label">'+it[1]+'</div>'; return; }
    var cur = it[0]===active;
    h += '<a class="an" href="admin-'+it[0]+'.html"'+(cur?' aria-current="true"':'')+'>'+
         '<svg><use href="#'+it[2]+'"/></svg><span>'+it[1]+'</span>'+
         (it[3]?'<span class="cnt'+(it[0]==="inbox"?" hot":"")+'">'+it[3]+'</span>':'')+'</a>';
  });
  h += '<div class="spacer"></div>';
  h += '<a class="adm-me" href="#" onclick="return false"><span class="av">АК</span><span class="nm"><b>Аброр Каримов</b><span>Мутахассиси САҲШ</span></span></a>';
  rail.innerHTML = h;
});
...
/* dynamic content (apply.html service switch, calendar, file rows, toasts) is set
   AFTER initial apply — re-translate it as it appears. ru→tg values aren't keys, so no loop. */
var _obs=null;
...
function i18nInit(){
  var l="tg"; try{ l=localStorage.getItem(I18N_KEY)||"tg"; }catch(e){}
